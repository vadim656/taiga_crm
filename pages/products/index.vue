...
const { result: products } = useQuery(ALL_PRODUCTS, () => ({
  PAGE: pageStart.value,
  PAGE_SIZE: 10
}))

const dataProductsMeta = computed(() => products.value?.products.meta.pagination.total ?? 1)
...
<template>
    ...
    <ClientOnly placeholder="Загрузка...">
      ...
      >
        ...
        <Column
          ...
          class="text-sm"
        ></Column>
        <Column field="id" header="Статус" style="width: 6%">
        ...
        <Column field="id" header="Действия" class="" style="width: 8%">
          <template #body="slotProps">
            <div class="flex items-center gap-2 justify-end">
              <button
  ...
  <ModalsModalEdit ref="modalEdit">
    ...
    <template #footer> <ButtonsBDelete>Сохранить</ButtonsBDelete></template>
  </ModalsModalEdit>
  <Dialog
